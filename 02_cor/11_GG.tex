...
\begin{itemize}
    ...
    \item Support de communication performant \GT{Pas trop certain de ce que cela veut dire, i.e., un peu vague: <<performant>>!}
    \item Utilisation d'outils adaptés
    \item Facilement versionnable
    \item Assez souple pour s'adapter \GT{s'adapter \`a quoi!?}
\end{itemize}
...
\ACOMPLETER{Une phrase sur les deux (trois?) sous-sections qui viennent...}
...
La figure~\ref{fig.sniper} pr\'esente un exemple d'une classe \texttt{Sniper1}, un objet de \emph{Mechanics} du GDD d\'efini avec le profil \emph{Game Genesis}.
...
\begin{figure}
    ...
    \caption{Exemple d'une classe \texttt{Sniper1} utilisant le stéréotype \texttt{SR}.}
    \label{fig.sniper}
\end{figure}
