\chapter{Un exemple d'application du profil \emph{Game Genesis}~: PUBG}
\label{chap.pubg}
...
\begin{figure}
    ...
    \caption[Les principaux stéréotypes de \emph{Game Genesis} utilisés pour PUBG.]{Les principaux stéréotypes de \emph{Game Genesis} utilisés dans notre exemple, le mod\`ele pour PUBG.}
...
\begin{figure}
    ...
    \caption[Un modèle UML utilisant \emph{GameGenesis} pour d\'ecrire PUBG.]{Un modèle UML (partiel) utilisant le profil \emph{GameGenesis} pour d\'ecrire les \'el\'ements de \emph{Mechanics} de PUBG.}
...
Ces éléments sont représentés dans un modèle UML utilisant \emph{Game Genesis} présenté à la Figure~\ref{fig.PUBG_model}~: 
\begin{itemize}
...
\gt{Tu dois utiliser le nous pour... <<je>>, par pour <<je et le lecteur>>.}
