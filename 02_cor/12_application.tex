...
PUBG (PlayerUnknown's BattleGround) est un jeu vidéo, sorti en 2017, qui compte des millions de joueurs à travers le globe. Développé par \emph{PUBG Corporation} (filiale de B\emph{luehole, Inc.}, plus récemment de \emph{Krafton Game Union}), PUBG est un des premiers <<\emph{Last man standing game} \emph{standalone},%
\footnote{Jeu \emph{standalone} (dans le cadre présent) : Jeu indépendant, qui n'est pas un contenu additionnel ou un module d'un autre jeu
\gt{Je ne comprends pas: Ind\'ependante de quoi? D'une machine? D'une plateforme de jeu?}}
%
avec  \emph{Fortnite} (\emph{Epic Games}),
permettant aux joueurs de participer à un >>. Ces deux jeux ont été les premiers \emph{standalone} à populariser le \emph{Battle Royale} et à le mettre à la portée de tous sans devoir développer ou installer du contenu additionnel dans des jeux existants.
...
Afin de pouvoir utiliser \emph{Game Genesis} dans un modèle il faut d'abord modéliser l'application du profil sur le modèle en question, c'est ce qui est représenté dans la Figure~\ref{fig.modelisation_application}.
...
- Pour la classe associative Attack, je ne vois toujours pas la ligne
  ...
  possible: PUBGAttack?  SoldierAttack?

- Je n'aime toujours pas le terme <<Element>>, trop generique!
  <<WorldElement>> serait-il possible?

- Je n'aime pas non plus <<Contain>> pour les Soldier dans Plane --
