...
Le \emph{framework} \glsfirst{mda} de Hunicke, Leblanc et Zubek~\cite{MDA_formal} tente de définir une structure pour accompagner un \emph{game designer} durant les étapes de documentation d'un jeu.
...
Les flèches de la figure~\ref{fig:mda} montrent l'interaction entre ces trois aspects. Selon l'article de Hunicke, LeBlanc et Zubek~\cite{MDA_formal}, un \emph{Game designer} met en place des \emph{Mechanics} dont découlent des \emph{Dynamics} dont naissent des \emph{Aesthetics}. Le joueur quant à lui interagit avec des \emph{Aesthetics}. Ces actions lui permettent de faire usage des \emph{Dynamics} afin de déclencher les éléments de \emph{Mechanics}.
...
Dans la figure~\ref{fig.dpe} les flèches représentent les relations entre le \emph{Game designer}, le jeu et le joueur. Le \emph{Game designer} \emph{Design} le jeu, le joueur joue au jeu (\emph{Play}) et il en résulte l'expérience de jeu (\emph{Experience}). 
La flèche allant d'\emph{Experience} vers \emph{Design} représente le processus itératif du design décrit par Salen et Zimmermann~\cite{Salen2013}, tel qu'illustr\'e \`a la figure~\ref{fig.dpe_iteratif}. 
...
Walk \emph{et al.} effectuent donc un découpage différent et intègrent de nouvelles notions dans les catégories, telles qu'illustr\'ees dans la figure~\ref{fig.dde_extended}. Dans cette figure les flèches entre les trois parties nous indiquent que nous gardons l'interaction présente entre les trois parties qui sont \emph{Design}, \emph{Dynamics} et \emph{Experience}. Au sein de la section \emph{Experience} les flèches indiquent que la combinaison de \emph{Senses}, \emph{Cerebellum} et \emph{Cerebrum} appliquée sur un joueur \textbf{en particulier} (\emph{Player<Subject>}) devient ce que l'on appelle la \emph{Perception}.
