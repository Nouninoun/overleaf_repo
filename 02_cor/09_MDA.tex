...
La conception de jeux est le processus de réflexion nécessaire à la création d'un jeu, vidéo ou non.
...
Un logiciel classique vise \`a répondre à un besoin précis identifié et réclamé par les utilisateurs.
...
Quant à lui, un jeu vidéo doit créer le besoin chez un utilisateur qui n'a pas de besoin liés à son utilisation.
Il doit proposer un but au joueur, lui donner l'envie de continuer à l'utiliser et doit générer des émotions pour être apprécié car il ne répond pas à une problématique.
Il est compliqué de définir une méthode précise pour décrire une émotion ainsi que les moyens devant être mis en place pour parvenir à la générer.
Le \emph{framework} \gls{mda}  met en place une structure pour accompagner un \emph{game designer} durant les étapes de documentation d'un jeu.
Le MDA essaie d'apporter une solution en apportant une catégorisation des émotions et les techniques pour parvenir à les générer.
...
Les \'el\'ements de l'aspect \emph{Aesthetics} sont, selon Hunicke, LeBlanc et Zubek~\cite{MDA_formal}, \guillemotleft~ce qui rend le jeu \emph{<<~fun~>>}. 
...
Le MDA permet donc de modéliser la conception du jeu vidéo en séparant les trois éléments suivants d'un jeu : les éléments de \emph{Mechanics, Dynamics et Aesthetics}.
Cependant de nombreux auteurs critiquent le MDA car certains genres de jeu ou certaines mécaniques de gameplay sont exclus de ce \emph{framework}.

C'est le cas du Framework <<~\emph{Design, Play, Experience}~>> (DPE), décrit dans la Section~\ref{sect.MDA_DPE}, qui cherche à intégrer les jeux reposant uniquement sur l'histoire, les jeux sérieux ou les outils d'apprentissage ludique dans leur framework.

C'est également le cas du Framework <<~\emph{Design, Dynamics, Experience}~>> (DDE), décrit dans la Section~\ref{sect.MDA_DDE}, qui lui avance que le MDA néglige certains aspects du design de jeux vidéos en se concentrant trop sur les éléments de \emph{mechanics}.
...
Les \'el\'ements associ\'es \`a l'aspect \emph{Design} sont les suivants~: 
    \begin{itemize}
        ...
        \item Interface : Toutes les mécaniques qui ont pour but de communiquer le jeu au joueur. Les graphismes, les sons, les réactions et interactions. Ces mécaniques peuvent aussi bien servir à effectuer une communication sous la forme Jeu~=>~Joueur, Joueur~=>~Jeu ou Jeu~=>~Jeu Cette partie comprend également les cinématiques, les textes affichés, et tout ce qu'il est possible de voir ou d'entendre dans le jeu.
    \end{itemize}
