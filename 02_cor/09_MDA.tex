...
Quant à lui, un jeu vidéo doit créer le besoin chez un utilisateur qui n'a pas de besoins liés à son utilisation.
...
Le \emph{framework} \gls{mda}  tente de mettre en place une structure pour accompagner un \emph{game designer} durant les étapes de documentation d'un jeu.
...
C'est le cas du \emph{framework} <<~\emph{Design, Play, Experience}~>> (DPE), décrit dans la Section~\ref{sect.MDA_DPE}, qui cherche à intégrer les jeux reposant uniquement sur l'histoire, les jeux sérieux ou les outils d'apprentissage ludique.

C'est également le cas du \emph{framework} <<~\emph{Design, Dynamics, Experience}~>> (DDE), décrit dans la Section~\ref{sect.MDA_DDE}, qui lui avance que MDA néglige certains aspects du design de jeux vidéos en se concentrant trop sur les éléments de \emph{Mechanics}.
