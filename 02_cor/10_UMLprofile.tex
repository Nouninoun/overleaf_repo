...
Dans la figure~\ref{fig.uml_but_definition} nous pouvons remarquer la présence d'une flêche noire à tête pleine entre les éléments \texttt{<<~stereotype~>> Bouton} et \texttt{<<~metaclass~>>Class}.
C'est une notation UML indiquant que \texttt{<<~metaclass~>>Class} est étendu par \texttt{<<~stereotype~>> Bouton}.
Cette notation d'extension peut inclure différents éléments qui ne sont pas requis dans notre cas comme :
\begin{itemize}
...
Cette contrainte énonce : Si l'état du bouton Marche est à False, alors l'état du bouton Action sera toujours égal à False.
